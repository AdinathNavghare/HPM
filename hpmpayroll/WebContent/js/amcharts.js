...
AmCharts.AmGraph = AmCharts.Class({
					...
					case "candlestick":
						if (!isNaN(ja) && !isNaN(R)) {
							...
							if (n) {
								...
								var la = new AmCharts.Cuboid(e, J, da, z, H, wa, Na, O, ka, U, ha, p, n, W),
								jb,
								kb;
								R > ja ? (jb = [C, Aa],
		...
		drawLineGraph : function (a, b, c, d) {
			...
			if (1 < a.length) {
				...
				q && (n = q);
				var q = e.fillColors,
				p = e.fillColorsSwitched;
				p && (q = p);
				var u = e.dashLength;
				(p = e.dashLengthSwitched) && (u = p);
				var p = e.negativeLineColor,
				r = e.negativeFillColors,
				...
				if (q) {
					...
					if (c.mouseIsOver || x || z || this.forceShow)
						if (this.forceShow = !1, z) {
							...
							0 < this.cursorAlpha && (v = this.line, u ? (t = 0, s = p + h) : (t = p, s = 0), A = this.animationDuration, 0 < A && !this.zooming ? isNaN(this.previousX) ? v.translate(t, s) : (v.translate(this.previousX, this.previousY), v.animate({
										translate : t + "," + s
									...
									for (t = 0; t < H.length; t++)
										if (f = H[t], (!this.oneBalloonOnly || f == B) && f.showBalloon && !f.hidden && f.balloonText && (w = q.axes[f.valueAxis.id].graphs[f.id], v = w.y, !isNaN(v))) {
											...
											s && (1 != this.graphBulletSize && AmCharts.isModern && (s = w.bulletGraphics) && (s.getBBox(), s.translate(w.bx, w.by, this.graphBulletSize), this.resizedBullets.push(w)), s = f.valueBalloon,
												E = c.getBalloonColor(f, w), s.setBounds(k, l, k + m, l + n), s.pointerOrientation = "H", s.changeColor(E), void 0 !== f.balloonAlpha && (s.fillAlpha = f.balloonAlpha), void 0 !== f.balloonTextColor && (s.color = f.balloonTextColor), s.setPosition(p + k, r + l), p = c.formatString(f.balloonText, w, f), (r = f.balloonFunction) && (p = r(w, f)), "" !== p && (u ? s.showBalloon(p) : (s.text = p, s.show = !0)), !u && s.set && s.set.hide(), h.push({
													yy : v,
													balloon : s
												}))
										}
		...
		draw : function () {
			...
			if (l = a.graph) {
				...
				for (p = 0; p < c.length; p++) {
					var u = c[p].axes[q.id].graphs[l.id].values,
					r;
					for (r in u)
			...
			if (!isNaN(a)) {
				...
				if (this.show) {
					...
					0 < this.animationDuration && !d && !isNaN(this.prevX) && (f.translate(this.prevX, this.prevY), f.animate({
							...
							"easeOutSine"), m && (E.left = this.prevTX + "px", E.top = this.prevTY + "px", this.xAnim = c.animate({
									node : m
