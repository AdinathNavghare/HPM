	...
	function createGraph(data, container) {
		...
		var tableData = {
			...
			yLegend: function() 
			{
				...
				{
					yLegend.unshift( Math.round(((chartYMax * i) / (yAxisMarkings - 1)) / 100000));           // for less amount or amount in thousand  yLegend.unshift(((chartYMax * i) / (yAxisMarkings - 1)) / 1000);
					if(Math.round(((chartYMax * i) / (yAxisMarkings - 1)) / 100000) >  yaxisdefault)
						{
						yaxisdefault= Math.round(((chartYMax * i) / (yAxisMarkings - 1)) / 100000);
						}
					
				}
				if(yaxisdefault==0)                        		    //when values are not available or ZERO
					{
					yLegend=[];
					for (var i = 0; i < yAxisMarkings; i++) 
					{
		...
		$.each(columnGroups, function(i) {
			...
			for (var j = 0, k = columnGroups[i].length; j < k; j++) {
				...
				barObj.bar = $('<div class="bar fig' + j + '"><div class="face front"></div><div class="face back"></div><div class="face left"></div><div class="face right"></div><div class="face top"></div><div class="face bottom"></div><span>' + addCommas(barObj.label) + '  <img src="images/rupee.png" height="10" width="10"></span></div>')
					.appendTo(barGroup);
				bars.push(barObj);
			}
		...
		$.each(yLegend, function(i) {			
			var listItem = $('<li><span>' + this + ' Lacs </span></li>')
				.appendTo(yAxisList);
		});
