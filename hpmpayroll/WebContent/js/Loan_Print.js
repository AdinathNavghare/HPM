function checkLoan() { 
	var loanamt = top.document.loan_form.loan_amt.value;
	var paymnt = top.document.loan_form.pay_month.value;
	var rate = top.document.loan_form.loan_per.value;
	...
	if(loanamt=="" || isNaN(parseFloat(loanamt)) || loanamt==0) { 
		...
		top.document.loan_form.loan_amt.value="";
		top.document.loan_form.loan_amt.focus();
		return false; 
	} else if(paymnt=="" || isNaN(parseFloat(paymnt)) || paymnt==0) {
		alert("Please enter a valid number of payments.");
		top.document.loan_form.pay_month.value="";
		top.document.loan_form.pay_month.focus();
		return false; 
	} else if(rate=="" || isNaN(parseFloat(rate)) || rate==0) {
		alert("Please enter the interest rate.");
		top.document.loan_form.loan_per.value="";
		top.document.loan_form.loan_per.focus();
		return false; 
	} else {
		show(); 
	}
...
function show() {
	var amount=parseFloat(top.document.loan_form.loan_amt.value);
	var numpay=parseInt(top.document.loan_form.pay_month.value);
	var rate=parseFloat(top.document.loan_form.loan_per.value);
